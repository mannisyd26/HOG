...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    if num_rolls == 0:
      score = 1+int(max((opponent_score-digit)/10, digit))
    else:
...
def max_scoring_num_rolls(dice=six_sided):
    ...
    10
    """
    rolls = make_averaged(roll_dice, 10)
    max_score = 0
    max_roll = 0
    n = 1
    while n<=10:
        print(str(n) + " dice scores " + str(rolls(n, dice)) + " on average")
        if max_score < rolls(n, dice):
            max_score = rolls(n, dice)
            max_roll = n
        n += 1
    return max_roll
...
def bacon_strategy(score, opponent_score):
    ...
    """
    digit = opponent_score%10
    if 1+int(max((opponent_score-digit)/10, digit)) >= BACON_MARGIN:
        return 0
    else:
...
def swap_strategy(score, opponent_score):
    ...
    digit = opponent_score%10
    if (score + (1+int(max((opponent_score-digit)/10, digit))))*2 == opponent_score:
        return 0
    elif score + (1+int(max((opponent_score-digit)/10, digit))) == opponent_score*2:
        return BASELINE_NUM_ROLLS
...
def final_strategy(score, opponent_score):
    ...
    This function takes the current player's score and the opponent's score as arguments and returns a number of rolls that would allow a greater success rate
    To start, I gave priority to any moves or number of rolls that would allow the player to reach 100 points and win. If the score plus the value added to
    the score after following the free bacon role is greater than 100, then the player should roll 0 dice to use the rule and win. Similarly, if the play is 1 point
    away from winning, rolling 10 dice creates a greater chance of rolling at least a 1 and winning. If the score is near 100, a certain number of rolls should be
    returned to increase the possibilty of reaching 100 points but also limit the risk of rolling a 1 and having the pig out rule take effect. The swine swap rule
    was also take into account where if the player's score after adding the "free bacon" or after adding 1 is half the value of the opponent_score, this would be
    a beneficial swap- meaning 0 or 10 dice should be rolls for the respective case. On the other hand, if the opponent_score is double the player score after
    any of these cases, a baseline number of rolls should be rolls to avoid giving the opponent an advantage The hog wild rule was also taken into account,
    where the free bacon or pig our rules shuld be used to ensure the opponent is more likely to get a four-sided die and thus more likely to roll a 1. Finally,
    when the players are both starting out with scores less than 10, rolling a baseline number of rolls makes it more likely for both to increase their scores
    without rolling a 1. If none of the conditions are met, a baseline number of rolls is returned.

    """

    dice = select_dice(score, opponent_score)
    digit = opponent_score%10
    added_bacon = 1+int(max((opponent_score-digit)/10, digit))

    "*** YOUR CODE HERE ***"
    # (planned_score + opponent_score) % 7 = 0 to give opponent four-sided dice (greater chance of rolling 1)
    #
    if (score+added_bacon) >= 100:
        return 0
    elif score == 99:
        return 10
    elif score>= 90:
        ...
        return 10
    elif (score+added_bacon)*2 == opponent_score:
        return 0
    elif (score+added_bacon) == opponent_score*2:
        return BASELINE_NUM_ROLLS
    elif (score+added_bacon+opponent_score) % 7 == 0:
        return 0
    elif (score+1+opponent_score) % 7 == 0:
        return 10
    elif score<10 and opponent_score<=10:
        return BASELINE_NUM_ROLLS
    else:
        return BASELINE_NUM_ROLLS
